...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--multiplier", type=int, default=10000, help="multiplier for model number"
    )
    ...
    for epoch in [
        args.multiplier * i for i in range(args.start_iter, args.end_iter + 1)
    ]:
        ...
        with torch.no_grad():
            for i in tqdm(range(args.n_sample // args.batch)):
                ...
                g_imgs = resize(
                    g_imgs, args.im_size
                )  # resize the image using given dimension
